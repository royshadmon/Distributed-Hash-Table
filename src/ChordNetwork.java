...
//        long[] time = new long[17];
        ...
        while (Math.pow(index, count) <= FingerTable.MAX_NODES) {
            List<Node> randomNodeList = ChordTester.generateRandomNodeList((int)Math.pow(index, count));
            ...
            Node n1 = randomNodeList.get(0);
            n1.join(null);

            for(int i = 1; i < randomNodeList.size(); i++) {
                Node n = randomNodeList.get(i);
                System.out.println("RANDOM NODE ID: " + n.getId());
                n.join(n1);
            }
